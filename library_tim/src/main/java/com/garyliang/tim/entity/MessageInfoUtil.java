...
public class MessageInfoUtil {
     ...
     * @return
     */
    public static List<MessageInfo> timmessages2Messageinfos(List<TIMMessage> timMessages, TIMConversationType isGroup, String groupID) {
        if (timMessages == null) {
        ...
        for (int i = 0; i < timMessages.size(); i++) {
            ...
            if (timMessage.getConversation().getType() == isGroup && isGroup == TIMConversationType.Group) {
                TUIKitLog.e("xx", "onNewMessages() Group 同一群消息 = ");
                if (groupID.equals(timMessage.getConversation().getPeer())) {
                    List<MessageInfo> info = timmessage2Messageinfo(timMessage, true);
                    if (info != null) {
